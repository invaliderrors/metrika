...
"Lowercased" is a `User_email_lowercase` CHECK, not a convention: `User_email_key` is a plain unique index and Postgres has no case-insensitive text type without `citext`, so `A@x.com` and `a@x.com` were measured inserting as two rows. The writer still normalises; the constraint is what makes forgetting it a refused write rather than a second account for the same person ([ADR-0040](./adr/0040-tenant-context-gucs.md) decision 8).
...
Three amendments from the migration that created the table ([ADR-0040](./adr/0040-tenant-context-gucs.md)). It also holds **`personalOwnerUserId`** — unique, nullable, restricted FK to `User` — because nothing else expresses "at most one personal organization per user", and a unique index on a nullable column is only half of that: Postgres treats NULLs as distinct, so the other half is the `Organization_personal_owner_required` CHECK (`kind <> 'PERSONAL' OR personalOwnerUserId IS NOT NULL`), hand-written because Prisma's DSL cannot express it. `countryCode` is `Char(2)` **plus** an `Organization_country_code_format` CHECK, because `Char(2)` is a maximum width and nothing more — `'c'` was measured being accepted and stored blank-padded. And `billingAddressId` is **not** in the schema: `Address` is not a Phase 1 table, and a nullable UUID with no referent is a fact nobody can check.

Note what the CHECK does **not** give you: it is one-directional, so a `TEAM` organization may hold a personal owner and a two-step `UPDATE` can leave a user with no personal organization at all. "Every user has a personal organization" is a product invariant, not a database one — read `User.personalOrganization` as genuinely nullable.
...
**Not every tenant-scoped table has an `organizationId`, and the tenancy vocabulary is three primitives rather than one.** `Organization`'s predicate is on `id`; `User` has no organization at all, because a person belongs to many; and the read that turns a verified provider `sub` into a `User` row runs before either tenant is known, so it has a primitive of its own. `app.current_user_id` and the `app.current_auth_provider` + `app.current_external_auth_id` pair are read through their own functions with the same deny-by-default property. Two of the five Phase 1A policies are deliberately **asymmetric** — the read half widened to a caller's memberships, the write half not — and a policy expression that reads another RLS-protected table has that table's policies applied to it, which bounds the widening and makes a cycle a query-time `42P17`.

Three things about that widening are worth carrying into every later phase, because each one was a measured hole in the first draft of those policies rather than a worry:

- **The bound is only as narrow as the table it reads through, and that table is one the app role WRITES.** `User`'s widened read goes through `OrganizationMember`, so a `WITH CHECK` there that constrained only `organizationId` let any member attach an arbitrary `userId` to their own roster and read that person's whole `User` row. **A `WITH CHECK` must constrain the tenant key AND every column any policy predicate reads.**
- **`DELETE` is filtered by `USING` and has no `WITH CHECK` half**, so a widened read is a widened delete. The usual mitigations — the soft-delete extension refusing hard deletes, and `onDelete: Restrict` children — apply only to soft-deletable models that have children. `OrganizationMember` is neither, which is why it carries the tree's one `AS RESTRICTIVE FOR DELETE` policy.
- **Nothing in a predicate anchors `app.current_org_id` to a membership row.** RLS consumes the primary control's output unchecked, so the helper that sets that GUC must derive it from a membership lookup and never from request input.

[ADR-0040](./adr/0040-tenant-context-gucs.md) has every predicate, every reason, every measurement, and the surprising consequence that a row's identifier must exist before its `INSERT`.
