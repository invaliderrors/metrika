...
| 0.11 | ◐ OpenTelemetry bootstrap in API and workers; correlation ID propagation across all three runtimes; Pino + structlog with the redaction list — built by Plan 0C, with all four log/error sinks behind the shared rule. Remaining: the two chain links that need a workflow, Temporal search attributes (ADR-0029 obligation 10), `@prisma/instrumentation`, span flush on SIGTERM, an exception RENDERER on the Python side, and the thrown-plain-object shape where `apps/api`'s two sinks still disagree | `apps/api/src/infrastructure/telemetry`, `apps/workers/packages/metrika_core`, `apps/web/src/lib/telemetry`, `packages/contracts/src/{redaction,sentry-event}.ts` |
...
`packages/contracts` that **four** log/error sinks derive from and grade
