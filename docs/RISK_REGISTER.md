...
_Correction, measured in Plan 0B-2 Task 2:_ ADR-0021 and an earlier draft of this entry said the three plugins "cannot be pinned directly, because `eslint-config-next` `require`s its own copies, so a direct declaration installs something the config never loads." That holds only when the direct declaration resolves to a **different version or a different peer context**. `packages/eslint-config/src/react.js` imports all three by name — it has to, and pnpm's isolated `node_modules` makes a transitive import impossible (`ERR_MODULE_NOT_FOUND`, measured) — and at the pinned versions under the same `eslint@10.8.0`, `fs.realpathSync` on both resolution paths returns the **same directory**: pnpm dedupes to one store entry. So the pins in `packages/eslint-config/package.json` are the copies `eslint-config-next` loads, not shadows of them. The original warning still applies to `apps/web`, which imports none of the three and would gain a manifest entry that lints nothing. **Reassess at the end of Phase 0B-2, and on any ESLint major.**
