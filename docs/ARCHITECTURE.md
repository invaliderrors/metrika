...
> **Status:** Blueprint. Implementation has started — Phase 0A and Plans 0B-1, 0B-2, 0B-3 and 0C have landed, so `apps/api` boots with an OpenTelemetry SDK and a redacting Pino sink, `apps/web` is a localised shell that originates a request ID, and `apps/workers` is a uv workspace with structlog, OTel and a Temporal tracing interceptor. Everything else here still describes the target, not the tree. [`ROADMAP.md`](./ROADMAP.md) is the authority on what exists today; where this document and the repository disagree, the repository is right and this document is stale.
...
| [CONTRACTS_AND_API.md](./CONTRACTS_AND_API.md)           | Zod-first contracts, `nestjs-zod`, REST design, errors, API client         |
...
| Contracts       | **Zod + `nestjs-zod` + `@nestjs/swagger`**                                                  | One Zod schema yields runtime validation, OpenAPI, server type-checking and — via `orval` — a typed TanStack Query client  | A codegen step and a less pleasant client than an inferred one. ts-rest was chosen first and failed its spike gate: see [ADR-0019](./adr/0019-nestjs-zod-contracts.md), superseding ADR-0009 |
...
| Observability   | **OpenTelemetry → Grafana Cloud**, plus Sentry                                              | One OTLP endpoint for traces/metrics/logs; Grafana-native as required. **Traces only today**, and no Grafana workspace yet | Free tier limits; Sentry overlaps slightly                                                                                                                                                   |
...
│   │   │   │   └── telemetry/              # OTel bootstrap, logger, redaction — metrics registry not built
...
│   ├── api-client/                   # orval-generated client + TanStack Query bindings
...
| `packages/api-client`        | Typed HTTP client generated by `orval` from the emitted OpenAPI document: auth injection, request IDs, retry policy, error normalisation, cancellation, TanStack Query hooks | `contracts`, `orval`, `@tanstack/react-query` | Prisma, Nest, `apps/*`                                                 | `createMetrikaClient(config)`, generated hooks                                        | Unit with MSW; contract tests against the API's generated OpenAPI                                  |
...
├── api/                      # transport: Nest controllers + Zod DTOs, SSE controllers. NO business logic
...
| `HealthModule`        | Liveness/readiness, dependency probes — Postgres only today; the rest join as modules need them              | —                                            | Infrastructure               |
...
graph LR
    ...
    Z --> O["OpenAPI 3.1<br/>nestjs-zod + @nestjs/swagger"]
...
- OpenTelemetry everywhere; one OTLP endpoint to Grafana Cloud for traces, metrics and logs. Sentry for error grouping and release health. **Built today: traces only, and only from `apps/api` and `apps/workers`.** There is no metrics pipeline and no OTLP log pipeline (`disableLogSending: true`, deliberately), no Grafana Cloud workspace has been contacted, and Sentry covers the API and the web app but **not** the workers — `sentry-sdk` is not installed there.
- **One correlation identity end to end:** `requestId` generated at the edge → OTel `traceId` → attached to the Temporal workflow as a search attribute → present in every worker log line → returned in every API error body. A customer support ticket with a request ID resolves to a complete trace across three runtimes. **The search-attribute hop does not exist**: ADR-0029 obligation 10 is undischarged and nothing provisions `MetrikaRequestId`. The edge, the log lines and the error body are built and asserted.
- Structured JSON logs (Pino, structlog) with an explicit redaction list: tokens, signed URLs, payment payloads, file names, and any geometry payload. **It is not a path list**, and copying one from an older revision of this document is the mistake [OBSERVABILITY.md](./OBSERVABILITY.md#redaction--an-allowlist-mindset) §3 exists to prevent: the list and the matching rule are both `packages/contracts/src/redaction.ts`, they cross into Python as generated code, and each sink contributes only its own traversal.
- Golden signals plus the business metrics from §103: upload success rate, analysis success rate and duration, slice success rate and duration, cache hit rate, quote generation duration, quote conversion rate, payment failure rate, and — from Phase 11 — estimate-vs-actual deviation per printer profile version. **None of these is emitted.** Nothing in the repository constructs a meter.
...
1. ~~**ts-rest viability**~~ — **RESOLVED, Phase 0.** The spike ran and ts-rest failed it: `@ts-rest/core` is hard-pinned to Zod 3 internals, its last publish of any kind was fourteen months earlier, and `@ts-rest/open-api` emitted silently empty schemas against Zod 4. The documented fallback was taken — `nestjs-zod` + `@nestjs/swagger` for validation and OpenAPI, `orval` for the client. See [ADR-0019](./adr/0019-nestjs-zod-contracts.md), which supersedes ADR-0009. The Zod schemas were unchanged by the switch, which is what made it reversible.
