...
- **TypeScript pinned to 6.0.3.** Not 7.x. `typescript-eslint@8.66.0` declares `peerDependencies.typescript: ">=4.8.4 <6.1.0"`, and TS 7.0.2 (the Go-port major, which `npm` currently tags `latest`) falls outside it — type-aware linting simply would not run, and type-aware linting is this project's substitute for code review. 6.0.3 is the newest version inside the supported range. Verified empirically against a real 6.0.3 install that `experimentalDecorators` + `emitDecoratorMetadata` still compile, so the NestJS path in Plan 0B is unaffected. **Fallback:** 5.9.3, if Plan 0B finds Prisma/Nest/Next incompatibility with 6.x.
- TS 6 makes `strict` default, drops ES5 and AMD/UMD/SystemJS targets, and removes `--moduleResolution node`, `baseUrl` and `outFile`. None of these affect `base.json`, which sets `module`/`moduleResolution: NodeNext` and `target: ES2023` explicitly. `types` now defaults to `[]`, which is why `node.json` must list `["node"]` explicitly.
