...
**Both halves of that pattern are load-bearing, and each was got wrong once before it was measured.** `(\.\./)+` rather than exactly two, because ARCHITECTURE.md §8 puts a feature's public surface at its root, so `src/features/models/index.ts` reaches a sibling's internals with a _single_ `../` — the one file every other feature imports through. And `(?!\.\.)` because `[^/]+` matches `..`, so without the guard the regex backtracks and rejects `../../hooks/use-model`: a nested component importing **its own feature's** hook.

Verified against eight path shapes — three cross-feature depths, three same-feature depths, a sibling's public surface, and a package import. The two-`../` form gets two wrong, the unguarded `(\.\./)+` form gets two wrong, the guarded form gets none. Fixture both ends: a boundary that rejects legitimate same-feature imports is disabled within a week.
