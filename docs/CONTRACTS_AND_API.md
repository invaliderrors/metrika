...
**The committed bytes are LF, and the emitter is what makes them so.** `datamodel-codegen` writes the module through Python's text mode, so on Windows it lands CRLF. Measured on a clean tree before the fix: a re-emit reported `273 insertions(+), 273 deletions(-)` on a file nobody had touched, while `git diff --ignore-cr-at-eol` reported nothing at all. There is no `.gitattributes` here, so a Windows contributor who commits that turns the `contracts` job red — CI re-emits LF on Linux — over a diff that is only line endings, in a file whose own header says `DO NOT EDIT`. `scripts/contracts-emit.mjs` therefore rewrites the generated file LF-only after the generator returns. It belongs there rather than in a git setting for the same reason as `--disable-timestamp`: the artefact under review must be a function of the schemas and the pinned `ruff`, not of who ran the command.
